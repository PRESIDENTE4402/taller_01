...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8 h-full">
        ...
        <div class="card bg-white shadow-lg border border-gray-100">
            <div class="card-body">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    ...
                    <a href="{{ route('panel.operaciones.ordenes_trabajo.index') }}" class="btn btn-outline h-auto py-4 flex flex-col gap-2 hover:scale-105 transition-transform border-gray-200 hover:border-gray-500 hover:bg-gray-50 text-gray-600 hover:text-gray-800">
                        <i class="fas fa-search text-2xl text-gray-500"></i>
                        <span>Buscar Orden</span>
                    </a>
                    <a href="{{ route('panel.vehiculos.index') }}" class="btn btn-outline btn-success h-auto py-4 flex flex-col gap-2 hover:scale-105 transition-transform border-gray-200 hover:border-emerald-500 hover:bg-emerald-50 text-gray-600 hover:text-emerald-600">
        ...
        <div class="card bg-white shadow-lg border border-gray-100 mb-6">
            <div class="card-body p-0">
                <div class="p-5 border-b border-gray-100 flex justify-between items-center bg-blue-50/50 rounded-t-2xl">
                    <h3 class="font-bold text-blue-900 flex items-center gap-2"><i class="fas fa-tools"></i> En Reparación / Taller</h3>
                </div>
                <div class="p-4 grid grid-cols-1 gap-4">
                    @forelse($ordenesEnProceso as $orden)
                    <div class="flex flex-col sm:flex-row gap-4 p-4 border border-gray-100 hover:border-blue-300 hover:shadow-md transition-all rounded-xl bg-white">
                        <div class="flex-1">
                            <div class="flex items-center gap-3 mb-2">
                                <span class="badge badge-warning text-white text-[10px] font-black tracking-widest uppercase">En Proceso</span>
                            ...
                            <div class="bg-gray-50 border-l-4 border-blue-400 p-3 rounded-r-lg mt-3">
                                <p class="text-xs text-gray-500 uppercase font-bold tracking-widest mb-1">Última Actualización:</p>
                                <p class="text-sm text-gray-700 italic">
                                    @if($orden->bitacoras->isNotEmpty())
                                    "{{ $orden->bitacoras->first()->descripcion }}"
                                    <span class="text-[10px] text-gray-400 block mt-1">- {{ $orden->bitacoras->first()->user->name ?? 'Taller' }} ({{ $orden->bitacoras->first()->tipo_actividad }})</span>
                                    @else
                        ...
                        <div class="flex items-center sm:border-l sm:border-gray-100 sm:pl-4">
                            <a href="{{ route('panel.operaciones.ordenes_trabajo.show', $orden->id) }}" class="btn btn-outline btn-primary btn-sm rounded-lg hover:scale-105 transition-transform"><i class="fas fa-arrow-right"></i> Ver Informe</a>
                        </div>
                    </div>
                    @empty
                    <div class="text-center text-gray-400 py-6 italic bg-gray-50/50 rounded-xl border border-dashed border-gray-200">
        ...
        <div class="card bg-white shadow-lg border border-gray-100 mb-6">
            <div class="card-body p-0">
                <div class="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50 rounded-t-2xl">
                    <h3 class="font-bold text-gray-700 flex items-center gap-2"><i class="fas fa-clipboard-list text-gray-500"></i> Pendientes de Revisión o Aprobación</h3>
                </div>
                <div class="p-4 grid grid-cols-1 gap-4">
                    @forelse($ordenesAbiertas as $orden)
                    <div class="flex flex-col sm:flex-row gap-4 p-4 border border-gray-200 hover:border-gray-400 hover:shadow-md transition-all rounded-xl bg-gray-50/30">
                        <div class="flex-1">
                            <div class="flex items-center gap-3 mb-2">
                                <span class="badge badge-info text-white text-[10px] font-black tracking-widest uppercase">Abierta</span>
                        ...
                        <div class="flex items-center sm:border-l sm:border-gray-200 sm:pl-4">
                            <a href="{{ route('panel.operaciones.ordenes_trabajo.show', $orden->id) }}" class="btn btn-outline btn-sm rounded-lg hover:scale-105 transition-transform">Ver Detalles</a>
                        </div>
                    </div>
                    @empty
        ...
        <div class="card bg-white shadow-lg border border-gray-100">
            <div class="card-body p-0">
                <div class="p-5 border-b border-gray-100 flex justify-between items-center bg-red-50/50 rounded-t-2xl">
                    <h3 class="font-bold text-red-800 flex items-center gap-2"><i class="fas fa-box-open"></i> En Espera de Repuestos</h3>
                </div>
                <div class="p-4 grid grid-cols-1 gap-4">
                    @forelse($ordenesEsperaRepuesto as $orden)
                    <div class="flex flex-col sm:flex-row gap-4 p-4 border border-red-100 hover:border-red-300 hover:shadow-md transition-all rounded-xl bg-white">
                        <div class="flex-1">
                            <div class="flex items-center gap-3 mb-2">
                                <span class="badge badge-error text-white text-[10px] font-black tracking-widest uppercase">Pausado</span>
                        ...
                        <div class="flex items-center sm:border-l sm:border-red-100 sm:pl-4">
                            <a href="{{ route('panel.operaciones.ordenes_trabajo.show', $orden->id) }}" class="btn btn-outline btn-error btn-sm rounded-lg hover:scale-105 transition-transform">Actualizar Estado</a>
                        </div>
                    </div>
                    @empty
                    <div class="text-center text-gray-400 py-6 italic border border-dashed border-gray-200 rounded-xl">
    ...
    <div class="col-span-1 space-y-6">
        ...
        <div class="card bg-white shadow-lg border border-gray-100">
            <div class="card-body p-5">
                <h3 class="card-title text-gray-700 mb-2 text-sm uppercase tracking-widest"><i class="fas fa-calendar-check text-blue-500"></i> PRÓXIMAS CITAS</h3>
                <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-4">Agendadas para hoy y mañana</p>

                <div class="space-y-3">
                    @forelse($citasProximas as $citaRow)
                    <div class="flex flex-col gap-1 p-3 bg-gray-50 border border-gray-100 rounded-xl">
                        <div class="flex justify-between items-center">
                            <span class="font-black text-gray-700 text-xs">{{ $citaRow->vehiculo->placa ?? 'N/A' }} <span class="text-gray-400 font-medium">| {{ $citaRow->vehiculo->marca->nombre ?? '' }}</span></span>
                            <span class="text-[9px] font-bold px-2 py-0.5 rounded-full {{ $citaRow->fecha_programada->isToday() ? 'bg-orange-100 text-orange-600' : 'bg-blue-100 text-blue-600' }}">
                                {{ $citaRow->fecha_programada->isToday() ? 'HOY' : 'MAÑANA' }} {{ $citaRow->fecha_programada->format('H:i') }}
                            </span>
                        </div>
                        <div class="flex justify-between items-end mt-1">
                            <span class="text-xs text-gray-500"><i class="fas fa-user text-[10px] mr-1"></i>{{ $citaRow->cliente->nombre_completo ?? '' }}</span>
                    ...
                    <div class="text-center p-6 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                        ...
                        <p class="text-xs text-gray-400 font-bold uppercase tracking-widest">No hay citas a corto plazo</p>
                    </div>
                    @endforelse
                </div>
            </div>
        ...
        <div class="card bg-white shadow-lg border border-gray-100">
            <div class="card-body">
                <h3 class="card-title text-gray-700 mb-4 text-sm font-bold uppercase tracking-widest"><i class="fas fa-bell text-yellow-500"></i> Avisos del Sistema</h3>

                <div class="alert alert-warning shadow-sm text-sm mb-3 bg-amber-50 text-amber-900 border-amber-200 py-2">
                    <i class="fas fa-store"></i>
                    <span><strong>Sucursal Actual:</strong> {{ session('sucursal_nombre') ?? (Auth::user()->sucursales->first()?->nombre ?? 'Principal') }}</span>
                </div>
