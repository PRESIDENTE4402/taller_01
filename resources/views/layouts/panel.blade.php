...
<body class="h-[100dvh] overflow-hidden flex text-gray-800 bg-gray-50 sidebar-body-wrapper transition-colors">
    ...
    <aside id="sidebar"
        ...
        <div class="h-16 flex items-center px-6 border-b border-slate-700 bg-slate-900 justify-between">
            <div
                class="flex items-center gap-3 font-bold text-xl tracking-tight text-white overflow-hidden whitespace-nowrap">
                <div
        ...
        <div class="flex-1 overflow-y-auto py-4 sidebar-scroll">
            <nav class="space-y-1 px-3">
                ...
                {{-- Sección Principal --}}
                <div
                    class="px-3 mb-2 text-xs font-bold text-slate-500 uppercase tracking-wider section-title whitespace-nowrap overflow-hidden">
                    Principal</div>

                @can('ver_dashboard')
                    <a href="{{ route('panel.dashboard') }}"
                        class="sidebar-menu-btn tooltip tooltip-right flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-md {{ request()->routeIs('panel.dashboard') ? 'sidebar-active' : 'sidebar-item text-slate-400' }} transition-colors"
                        data-tip="Dashboard">
                ...
                <a href="{{ route('home') }}" target="_blank"
                    class="sidebar-menu-btn tooltip tooltip-right flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-md {{ request()->routeIs('home') ? 'sidebar-active' : 'sidebar-item text-slate-400' }} transition-colors"
                    data-tip="Página Web">
                ...
                @if(Gate::check('gestionar_citas') || Gate::check('gestionar_recepcion') || Gate::check('gestionar_ordenes_trabajo') || auth()->user()->hasRole('mecanico') || auth()->user()->hasRole('ayudante') || auth()->user()->hasRole('tecnico'))
                    ...
                    @can('gestionar_citas')
                        <a href="{{ route('panel.operaciones.citas.index') }}"
                            class="sidebar-menu-btn tooltip tooltip-right flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-md {{ request()->routeIs('panel.operaciones.citas.*') ? 'sidebar-active' : 'sidebar-item text-slate-400' }} transition-colors"
                            data-tip="Gestionar Citas">
                    ...
                    @can('gestionar_recepcion')
                        <a href="{{ route('panel.operaciones.ordenes_trabajo.dashboard') }}"
                            class="sidebar-menu-btn tooltip tooltip-right flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-md {{ request()->routeIs('panel.operaciones.ordenes_trabajo.dashboard') ? 'sidebar-active' : 'sidebar-item text-slate-400' }} transition-colors"
                            data-tip="Recepción Vehículos">
                            <i class="fas fa-car w-5 text-center flex-shrink-0"></i>
                    ...
                    @can('gestionar_ordenes_trabajo')
                        ...
                        <a href="{{ route('panel.colaboradores.index') }}"
                            class="sidebar-menu-btn tooltip tooltip-right flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-md {{ request()->routeIs('panel.colaboradores.*') ? 'sidebar-active' : 'sidebar-item text-slate-400' }} transition-colors"
                            data-tip="Tablero de Personal">
                    ...
                    @if(auth()->user()->hasRole('mecanico') || auth()->user()->hasRole('ayudante') || auth()->user()->hasRole('tecnico') || auth()->user()->hasRole('admin') || auth()->user()->hasRole('recepcionista'))
                        <a href="{{ route('panel.mis_tareas.index') }}"
                            class="sidebar-menu-btn tooltip tooltip-right flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-md {{ request()->routeIs('panel.mis_tareas.*') ? 'sidebar-active' : 'sidebar-item text-slate-400' }} transition-colors"
                            data-tip="{{ (auth()->user()->hasRole('admin') || auth()->user()->hasRole('recepcionista')) ? 'Tareas de Personal' : 'Mis Tareas' }}">
                            <i class="fas fa-tools w-5 text-center flex-shrink-0"></i>
                            <span
                                class="menu-text whitespace-nowrap overflow-hidden text-ellipsis">{{ (auth()->user()->hasRole('admin') || auth()->user()->hasRole('recepcionista')) ? 'Tareas de Personal' : 'Mis Tareas' }}</span>
                        </a>
                    @endif
                @endif
                ...
                @can('gestionar_inventario')
                    ...
                    <a href="{{ route('panel.mantenimientos.categorias.index') }}"
                        class="sidebar-menu-btn tooltip tooltip-right flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-md {{ request()->routeIs('panel.mantenimientos.categorias.*') ? 'sidebar-active' : 'sidebar-item text-slate-400' }} transition-colors"
                        data-tip="Categorías">
                ...
                @if(Gate::check('gestionar_marcas') || Gate::check('gestionar_versiones') || Gate::check('gestionar_sucursales') || Gate::check('gestionar_imagenes_landing') || Gate::check('gestionar_items_recepcion'))
                    ...
                    @can('gestionar_marcas')
                        <a href="{{ route('panel.mantenimientos.marcas.index') }}"
                            class="sidebar-menu-btn tooltip tooltip-right flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-md {{ request()->routeIs('panel.mantenimientos.marcas.*') ? 'sidebar-active' : 'sidebar-item text-slate-400' }} transition-colors"
                            data-tip="Marcas de Vehículos">
                            <i class="fas fa-tags w-5 text-center flex-shrink-0"></i>
                    ...
                    @can('gestionar_versiones')
                        <a href="{{ route('panel.mantenimientos.versiones.index') }}"
                            class="sidebar-menu-btn tooltip tooltip-right flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-md {{ request()->routeIs('panel.mantenimientos.versiones.*') ? 'sidebar-active' : 'sidebar-item text-slate-400' }} transition-colors"
                            data-tip="Versiones de Modelos">
                    ...
                    @can('gestionar_sucursales')
                        <a href="{{ route('panel.mantenimientos.sucursales.index') }}"
                            class="sidebar-menu-btn tooltip tooltip-right flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-md {{ request()->routeIs('panel.mantenimientos.sucursales.*') ? 'sidebar-active' : 'sidebar-item text-slate-400' }} transition-colors"
                            data-tip="Sucursales">
                ...
                @if(Gate::check('ver_mi_qr') || Gate::check('ver_asistencias'))
                    ...
                    @can('ver_mi_qr')
                        <a href="{{ route('panel.rrhh.asistencias.mi-qr') }}"
                            class="sidebar-menu-btn tooltip tooltip-right flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-md {{ request()->routeIs('panel.rrhh.asistencias.mi-qr') ? 'sidebar-active' : 'sidebar-item text-slate-400' }} transition-colors"
                            data-tip="Mi Credencial QR">
                    ...
                    @can('ver_asistencias')
                        <a href="{{ route('panel.rrhh.asistencias.index') }}"
                            class="sidebar-menu-btn tooltip tooltip-right flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-md {{ request()->routeIs('panel.rrhh.asistencias.index') ? 'sidebar-active' : 'sidebar-item text-slate-400' }} transition-colors"
                            data-tip="Control Asistencias">
                            <i class="fas fa-fingerprint w-5 text-center flex-shrink-0"></i>
                            <span class="menu-text whitespace-nowrap overflow-hidden text-ellipsis">Control Asistencias
                                (Admin)</span>
                        </a>
                    @endcan
                @endif
                ...
                @if(Gate::check('gestionar_planilla') || auth()->user()->hasRole('admin'))
                    ...
                    <a href="{{ route('panel.planilla.pagos.index') }}"
                        class="sidebar-menu-btn tooltip tooltip-right flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-md {{ request()->routeIs('panel.planilla.pagos.*') ? 'sidebar-active' : 'sidebar-item text-slate-400' }} transition-colors"
                        data-tip="Pago a Trabajadores">
                ...
                @if(Gate::check('gestionar_roles') || Gate::check('gestionar_permisos') || Gate::check('gestionar_usuarios'))
                    ...
                    @can('gestionar_roles')
                        <a href="{{ route('panel.seguridad.roles.index') }}"
                            class="sidebar-menu-btn tooltip tooltip-right flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-md {{ request()->routeIs('panel.seguridad.roles.*') ? 'sidebar-active' : 'sidebar-item text-slate-400' }} transition-colors"
                            data-tip="Roles">
                    ...
                    @can('gestionar_permisos')
                        <a href="{{ route('panel.seguridad.permisos.index') }}"
                            class="sidebar-menu-btn tooltip tooltip-right flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-md {{ request()->routeIs('panel.seguridad.permisos.*') ? 'sidebar-active' : 'sidebar-item text-slate-400' }} transition-colors"
                            data-tip="Permisos">
                    ...
                    @can('gestionar_usuarios')
                        <a href="{{ route('panel.seguridad.usuarios.index') }}"
                            class="sidebar-menu-btn tooltip tooltip-right flex items-center gap-3 px-3 py-2 text-sm font-medium rounded-md {{ request()->routeIs('panel.seguridad.usuarios.*') ? 'sidebar-active' : 'sidebar-item text-slate-400' }} transition-colors"
                            data-tip="Usuarios">
                            <i class="fas fa-users-cog w-5 text-center flex-shrink-0"></i>
        ...
        {{-- User Footer --}}
        <div
            class="border-t border-slate-700 p-4 pb-6 sm:pb-4 bg-slate-900/50 flex flex-col items-center flex-shrink-0">
            <div class="flex items-center gap-3 w-full justify-center md:justify-start">
                <div class="avatar online placeholder flex-shrink-0">
                    <div
                        class="bg-blue-600 text-white rounded-full w-10 h-10 shadow-md flex items-center justify-center">
