...
@section('content')
            ...
            <div class="card bg-white shadow-sm border border-slate-200">
                <div class="card-body items-center text-center">
                    <div class="avatar placeholder mb-2">
                        <div
                            class="bg-blue-600 text-white rounded-full w-24 text-3xl shadow-lg ring ring-blue-50 ring-offset-2">
                            <span>{{ substr($cliente->nombre_completo, 0, 1) }}</span>
                        </div>
                    </div>
                    <h2 id="clientName" class="card-title text-slate-800">{{ $cliente->nombre_completo }}</h2>
                    <div id="clientBadge"
                        class="badge badge-lg {{ $cliente->es_empresa ? 'badge-primary' : 'badge-ghost' }} gap-2">
                    ...
                    <div class="w-full text-left space-y-3">
                        <div id="infoEmpresaBlock"
                            class="flex items-start gap-3 {{ $cliente->es_empresa ? '' : 'hidden' }}">
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p id="clientEmail" class="text-sm font-medium text-blue-600 truncate">
                                    {{ $cliente->email ?: 'Sin email' }}</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p id="clientAddress" class="text-sm font-medium text-slate-600">
                                    {{ $cliente->direccion ?: 'Sin dirección registrada' }}</p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card-body p-5">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <a href="{{ route('panel.operaciones.citas.index') }}"
                            class="btn btn-sm btn-outline btn-info gap-2 h-auto py-2">
                            <i class="fas fa-plus"></i> Cita
                        </a>
                        <a href="{{ route('panel.operaciones.ordenes_trabajo.create', ['cliente_id' => $cliente->id]) }}"
                            class="btn btn-sm btn-outline btn-warning gap-2 h-auto py-2">
            ...
            <div class="card bg-white shadow-sm border border-slate-200">
                ...
                <div class="card-body p-0">
                    <div id="vehiclesTableContainer"
                        class="overflow-x-auto {{ $cliente->vehiculos->count() > 0 ? '' : 'hidden' }}">
                            ...
                            <tbody id="vehiclesTableBody">
                                @foreach($cliente->vehiculos as $vehiculo)
                                    <tr class="hover" id="vehicle-row-{{ $vehiculo->id }}">
                                        <td>
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <div class="text-xs opacity-50">{{ $vehiculo->modelo->nombre ?? 'N/A' }}
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
                                        <td><span class="badge badge-outline font-mono font-bold">{{ $vehiculo->placa }}</span>
                                        </td>
                                        <td>{{ $vehiculo->anio }}</td>
                                        ...
                                        <td class="text-right">
                                            <a href="{{ route('panel.vehiculos.show', $vehiculo->id) }}"
                                                class="btn btn-ghost btn-xs text-info" title="Ver Perfil del Vehículo"><i
                                                    class="fas fa-eye"></i></a>
                                            <button class="btn btn-ghost btn-xs text-blue-600 btn-edit-vehicle"
                                                data-vehicle="{{ json_encode($vehiculo) }}"><i class="fas fa-edit"></i></button>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                    ...
                    <div id="vehiclesEmptyState"
                        class="p-8 text-center text-slate-500 {{ $cliente->vehiculos->count() > 0 ? 'hidden' : '' }}">
                        ...
                        <button class="btn btn-sm btn-primary" id="btnRegisterFirstVehicle">Registrar Primer
                            Vehículo</button>
                    </div>
                </div>
            </div>
                                ...
                                @foreach($cliente->ordenes as $orden)
                                    <tr>
                                        <td class="font-mono font-bold text-blue-600">
                                            #{{ str_pad($orden->id, 5, '0', STR_PAD_LEFT) }}</td>
                                        <td>{{ $orden->created_at->format('d/m/Y') }}</td>
                                        <td>{{ $orden->vehiculo->placa ?? 'N/A' }}</td>
                                        <td>
                                            <div class="badge badge-sm badge-{{ $orden->estado_color ?? 'primary' }}">
                                                {{ ucfirst($orden->estado) }}</div>
                                        </td>
                            ...
                            @foreach($cliente->citas->sortByDesc('created_at')->take(5) as $cita)
                                <li class="step {{ $cita->estado == 'concretada' ? 'step-primary' : '' }}">
                                    <div class="text-left w-full ml-4 mb-4">
                                        <p class="font-bold">{{ $cita->fecha_programada->format('d/m/Y H:i') }} - <span
                                                class="badge badge-sm">{{ $cita->estado }}</span></p>
    ...
    <div id="clientModal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog"
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div id="clientModalBackdrop" class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity"
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-[10px] text-gray-400 mt-1">Habilita la consulta del historial en la web.</p>
                                </div>
                            </div>
    ...
    <div id="vehicleModal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog"
        aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div id="vehicleModalBackdrop" class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity"
                aria-hidden="true"></div>
            <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
            <div
                class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <form id="vehicleForm">
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <h3 class="text-lg leading-6 font-medium text-gray-900 mb-4" id="vehicleModalTitle">Nuevo Vehículo
                        </h3>
                        ...
                        <div class="grid grid-cols-1 gap-4">
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div class="col-span-2">
                                    <label class="block text-sm font-medium text-gray-700">Versión <span
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="number" name="anio" id="anio" required min="1900" max="{{ date('Y') + 1 }}"
                                        value="{{ date('Y') }}"
                                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                </div>
                            </div>
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        ...
                        <button type="button" id="btnCancelVehicle"
                            class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
                            Cancelar
                        </button>
                    </div>
    ...
    <div id="client-data" data-es-empresa="{{ $cliente->es_empresa ? 'true' : 'false' }}"
        data-client-base="{{ url('panel/clientes') }}" data-vehicle-store="{{ route('panel.vehiculos.store') }}"
        data-vehicle-base="{{ url('panel/vehiculos') }}" data-get-brands="{{ route('panel.operaciones.citas.getBrands') }}"
